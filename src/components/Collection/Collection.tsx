...
function Collection<T extends object>({
  ...
  const searchInputRefs = columns.map(() => React.createRef<Input>());

  const transformedColumns = columns.map((column: CustomColumnsProps<T>, index: number) => {
      ...
      return {
        ...
        filterDropdown: ({
          ...
          <div className={styles.filterDropdown}>
            ...
            >
              <FormattedMessage key="Input.Search" defaultMessage="Keresés" />
            </Button>
            <Button onClick={clearFilters} size="small" className={styles.filterDropdownButton}>
              <FormattedMessage key="Input.Reset" defaultMessage="Törlés" />
            </Button>
          </div>
        ),
        // eslint-disable-next-line react/display-name
        ...
        onFilterDropdownVisibleChange: (visible: boolean) => {
          if (visible) {
            setTimeout(() => {
              searchInputRefs[index]?.current?.select();
            });
          }
        },
    ...
    if (searchType === 'datetimerange' || searchType === 'daterange') {
      return {
        ...
        onFilterDropdownVisibleChange: (visible: boolean) => {
          if (visible) {
            setTimeout(() => searchInputRefs[index].current?.select());
          }
        },
